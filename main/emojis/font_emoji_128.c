...
static const void* get_imgfont_path(const lv_font_t * font, uint32_t unicode, uint32_t unicode_next, int32_t * offset_y, void * user_data) {
    ...
    for (size_t i = 0; i < sizeof(emoji_128_table) / sizeof(emoji_128_table[0]); i++) {
        if (emoji_128_table[i].unicode == unicode) {
            return emoji_128_table[i].emoji;
        }
    }
    return NULL;
}
...
const lv_font_t* font_emoji_128_init(void) {
